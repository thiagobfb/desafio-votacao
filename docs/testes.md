...
> Cobertura atual: **60 testes**, todos verdes (`mvn verify`).

## Tipos de teste presentes

| Tipo | Anotação Spring | Quantos | O que cobre |
|---|---|---|---|
| **Unitário** (service) | — | 22 | Regras de negócio (RN-1..RN-5) — Mockito + AssertJ |
| **Slice de persistência** | `@DataJpaTest` + Flyway | 7 | Entidades, constraints UNIQUE, queries derivadas |
| **Slice web** | `@WebMvcTest` + MockMvc | 24 | Controllers — status codes, validação, formato de erro |
...
├── voto/
│   ├── service/VotoServiceTest          (5)
│   └── api/VotoControllerTest           (7)
├── resultado/
